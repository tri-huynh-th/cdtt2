<?php
require_once $_SERVER['DOCUMENT_ROOT'] . '/blogwebsite/includes/admin-header.php';

// Xử lý thêm bài viết
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $title = $_POST['title'] ?? '';
    $overview = $_POST['overview'] ?? '';
    $category = $_POST['category'] ?? ''; // Lấy giá trị category từ form
    ...
    if (empty($title)) {
    ...
    } elseif (empty($category)) { // Thêm kiểm tra cho category
        setAlert('danger', 'Vui lòng chọn danh mục bài viết');
    } else {
        // Upload thumbnail
        $thumbnail = '';
        if (isset($_FILES['thumbnail']) && $_FILES['thumbnail']['error'] == 0) {
            ...
            if (!$thumbnail) {
                setAlert('danger', 'Lỗi khi tải lên hình ảnh thumbnail. Vui lòng thử lại.');
                $thumbnail = '';
            }
        } else {
            setAlert('danger', 'Vui lòng chọn hình ảnh thumbnail cho bài viết');
        }
        
        if (!empty($thumbnail)) {
            // Thêm bài viết
            // Cập nhật câu lệnh INSERT để thêm cột category
            $stmt = $conn->prepare("INSERT INTO posts (title, thumbnail, overview, category) VALUES (?, ?, ?, ?)");
            // Cập nhật bind_param để thêm tham số category (kiểu string 's')
            $stmt->bind_param("ssss", $title, $thumbnail, $overview, $category); // Thêm 's' cho $category
            ...
            if ($stmt->execute()) {
                ...
                if (count($ingredient_names) > 0) {
                    $stmt_ingredients = $conn->prepare("INSERT INTO ingredients (post_id, name, amount) VALUES (?, ?, ?)");
                    
                    for ($i = 0; $i < count($ingredient_names); $i++) {
                        if (!empty($ingredient_names[$i]) && !empty($ingredient_amounts[$i])) {
                            $stmt_ingredients->bind_param("iss", $post_id, $ingredient_names[$i], $ingredient_amounts[$i]);
                            $stmt_ingredients->execute();
                        }
                    }
                    $stmt_ingredients->close(); // Đóng statement sau khi dùng xong
                }
                ...
                if (count($step_descriptions) > 0) {
                    $stmt_steps = $conn->prepare("INSERT INTO steps (post_id, description, image, step_order) VALUES (?, ?, ?, ?)");
                    
                    for ($i = 0; $i < count($step_descriptions); $i++) {
                        if (!empty($step_descriptions[$i])) {
                            // Upload hình ảnh cho bước (nếu có)
                            $step_image = '';
                            // Đảm bảo chỉ xử lý file nếu nó tồn tại và không có lỗi
                            if (isset($_FILES['step_image']['name'][$i]) && !empty($_FILES['step_image']['name'][$i]) && $_FILES['step_image']['error'][$i] == 0) {
                                $file = [
                            ...
                            $step_order = $i + 1;
                            $stmt_steps->bind_param("issi", $post_id, $step_descriptions[$i], $step_image, $step_order);
                            $stmt_steps->execute();
                        }
                    }
                    $stmt_steps->close(); // Đóng statement sau khi dùng xong
                }
                ...
                setAlert('danger', 'Đã xảy ra lỗi khi thêm bài viết');
            }
            $stmt->close(); // Đóng statement cho posts
        }
    }
}
?>
...
<?php require_once $_SERVER['DOCUMENT_ROOT'] . '/blogwebsite/includes/admin-footer.php'; ?>